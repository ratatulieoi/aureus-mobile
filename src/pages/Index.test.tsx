...
describe('Index mobile navigation and dashboard', () => {
  ...
  it('opens Backup directly and keeps Others available as a primary destination', async () => {
    const user = userEvent.setup();
    render(<Index />);
    ...
    await user.click(screen.getByRole('button', { name: 'Buka menu Aureus' }));
    await user.click(screen.getByRole('menuitem', { name: /^Others/ }));
    expect(await screen.findByRole('heading', { name: 'Others' })).toBeInTheDocument();
  });
