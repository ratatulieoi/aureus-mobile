...
const MonthlyReports: React.FC<MonthlyReportsProps> = ({ transactions }) => {
  ...
  return (
    <Card>
      ...
      <CardContent>
        <div className="grid min-w-0 grid-cols-1 gap-3 min-[360px]:grid-cols-2 sm:grid-cols-[160px_120px_minmax(0,1fr)] sm:items-end">
          <div className="min-w-0"><Label htmlFor="report-month" className="mb-1 block text-xs text-muted-foreground">Bulan laporan</Label><Select value={selectedMonth.toString()} onValueChange={(value) => setSelectedMonth(Number(value))}><SelectTrigger id="report-month" className="h-11 w-full min-w-0"><SelectValue /></SelectTrigger><SelectContent>{MONTHS.map((month, index) => <SelectItem key={month} value={index.toString()}>{month}</SelectItem>)}</SelectContent></Select></div>
          <div className="min-w-0"><Label htmlFor="report-year" className="mb-1 block text-xs text-muted-foreground">Tahun laporan</Label><Select value={selectedYear.toString()} onValueChange={(value) => setSelectedYear(Number(value))}><SelectTrigger id="report-year" className="h-11 w-full min-w-0"><SelectValue /></SelectTrigger><SelectContent>{availableYears.map((year) => <SelectItem key={year} value={year.toString()}>{year}</SelectItem>)}</SelectContent></Select></div>
          <div className="flex min-w-0 gap-2 min-[360px]:col-span-2 sm:col-span-1 sm:ml-auto"><Button onClick={downloadCSV} variant="outline" className="h-11 min-w-0 flex-1 gap-2 sm:flex-none"><ArrowDown aria-hidden="true" className="h-4 w-4" />Ekspor CSV</Button><Button onClick={downloadPDF} variant="outline" className="h-11 min-w-0 flex-1 gap-2 sm:flex-none"><ArrowDown aria-hidden="true" className="h-4 w-4" />Cetak PDF</Button></div>
        </div>
