...
test('accepts an explicit positive manual version code and signed mode', () => {
  const output = run({ EVENT_NAME: 'workflow_dispatch', INPUT_VERSION_NAME: '3.0.0', INPUT_VERSION_CODE: '3000000', INPUT_SIGNING: 'signed' });
  assert.match(output, /version_name=3.0.0/);
  assert.match(output, /signing=signed/);
});

test('rejects malformed tags and invalid manual codes', () => {
  for (const env of [
    { EVENT_NAME: 'push', TAG_NAME: 'release-1.0.0' },
    { EVENT_NAME: 'workflow_dispatch', INPUT_VERSION_NAME: '1.0.0', INPUT_VERSION_CODE: '0', INPUT_SIGNING: 'unsigned' },
  ]) {
